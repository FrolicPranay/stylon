...
getEffectiveMax() {
  ...
  const stock = parseIntOrDefault(this.refs.quantityInput.getAttribute('data-stock'), null);

  // Stock cap: use raw stock, do NOT subtract cartQuantity
  const stockMax = stock !== null ? Math.max(stock, min) : null;

  if (max === null && stockMax === null) return null;
  if (max === null) return stockMax;
  if (stockMax === null) return Math.max(max - cartQuantity, min);
  return Math.min(Math.max(max - cartQuantity, min), stockMax);
}
  ...
  updateButtonStates() {
    ...
    // Stock limit message
const stock = parseIntOrDefault(this.refs.quantityInput.getAttribute('data-stock'), null);
const stockMsg = this.querySelector('[data-stock-message]');
if (stockMsg && stock !== null && value >= stock) {
  stockMsg.textContent = `Only ${stock} items available.`;
  stockMsg.hidden = false;
} else if (stockMsg) {
  stockMsg.hidden = true;
}
